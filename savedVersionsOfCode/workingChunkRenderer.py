...
void main() {
    ...
    uint blocksOnX = uint((instance_scale >> bitsToShift) & andValue);
    uint blocksOnY = uint((instance_scale >> 0u) & andValue);
    ...
    // use this to not repeat the texture ; for if you want to visualize the greedy meshing
    //scaled_texcoord = in_texcoord;
...
flat in uint texture_layer;

void main() {
    //float layerF = mod(frag_texcoord.x, 28.0);  // example layer selection
    //uint layer = uint(layerF);
    uint layer = 0u; // For something later? YES FOR THE SUPER GREEDY MESHING
    
    color = texture( texArray, vec3(frag_texcoord, texture_layer + layer) );
}
"""
