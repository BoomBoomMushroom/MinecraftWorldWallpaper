...
void main() {
    ...
    uint scaleX = uint((instance_scale >> 8) & 0xFFu);
    uint scaleY = uint((instance_scale >> 0) & 0xFFu);
    ...
    uint blocksOnX = ((scaleX - 1u) / 2u) + 1u;
    uint blocksOnY = ((scaleY - 1u) / 2u) + 1u;
    
    vec2 scaled_texcoord = vec2(
        in_texcoord.x * float(blocksOnX),
        in_texcoord.y * float(blocksOnY)
    );
...
def greedyMeshBlocks(blocks: list[Block]):
    faces = []
...
def executeGreedyMeshOnBlock(block):
    faces = greedyMeshBlocks([block])
    return faces

def distributeGreedyMeshAlgorithm(blocks):
    updateBlockMap(blocks)
    
    with ThreadPoolExecutor() as executor:
        listOfFaceResults = list(executor.map(executeGreedyMeshOnBlock, blocks))
        faces = []
        for faceResult in listOfFaceResults:
            faces.extend(faceResult)
        generateTextureAndInstanceDataFromFaceArray(faces)
        
        print("done doing greedy mesh on the blocks!")
        return len(faces)
...
def renderFrame():
    ...
    for texturePath in textureAndInstanceData.keys():
        ...
        scaleBuffer = ctx.buffer( scalesBytes )
        ...
        vao = ctx.vertex_array(
            ...
            [
                ...
                (scaleBuffer, '1u/i', 'instance_scale'),
            ],
            quadIbo
        )
